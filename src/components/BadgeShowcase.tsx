...
import { BADGE_DEFINITIONS, getBadgeByKey } from "@/lib/badges";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { Lock } from "lucide-react";

interface BadgeShowcaseProps {
  userId: string;
  showLocked?: boolean; // show unearned badges grayed out (for own profile)
}
...
export default function BadgeShowcase({ userId, showLocked = false }: BadgeShowcaseProps) {
  ...
  // On own profile, auto-recalculate on mount
  useEffect(() => {
    if (user?.id === userId) {
      recalculate();
    }
  ...
  const badges = showLocked
    ? BADGE_DEFINITIONS
    : BADGE_DEFINITIONS.filter(b => earnedKeys.has(b.key));
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
        🏆 Insígnias {showLocked && <span className="text-xs text-muted-foreground font-normal">({earnedKeys.size}/{BADGE_DEFINITIONS.length})</span>}
      </h3>
